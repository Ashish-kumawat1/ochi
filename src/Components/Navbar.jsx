    ...
    <div className={`fixed z-[999] backdrop-blur bg-[rgba(255,255,255,0.03)] w-full px-20 py-5 f-["NeueMontreal"] flex justify-between items-center transition-transform duration-300 ease-in-out ${visible ? 'translate-y-0' : '-translate-y-full'}`}>
        ...
        <div className="links flex gap-8 ">
          {['Services ','Our work','About us','Insights','Contact us'].map((item,index)=>(
            <a key={index} className={`text-lg font-light ${index===4 && "ml-44" } relative after:content-[''] after:absolute after:bottom-0 after:left-0 after:w-0 after:h-[2px] after:bg-white/10 after:duration-300 hover:after:w-full  hover:after:ease-in-out `}>{item}</a>
          ))}
          </div>    
